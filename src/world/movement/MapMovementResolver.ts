...
export class MapMovementResolver implements MovementResolver {
    ...
    moveCircle(body: MovableCircle, deltaX: number, deltaY: number): void {
        ...
        const minY = bounds.top + body.radius;

        const maxY = bounds.bottom - body.radius;

        body.x = clamp(body.x + deltaX, minX, maxX);

        body.y = clamp(body.y + deltaY, minY, maxY);
    }
}
